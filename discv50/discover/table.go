...
func newTable(t transport, db *enode.DB, revalidator func(node2 *enode.Node) (uint64, error),
	onSeen func(n *enode.Node, at time.Time, liveness uint),
	bootnodes []*enode.Node, log log.Logger) (*Table, error) {
	tab := &Table{
		net:         t,
		db:          db,
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	remoteSeq, err := tab.revalidator(unwrapNode(last))
