...
func newUDPv5(conn UDPConn, ln *enode.LocalNode, cfg Config) (*UDPv5, error) {
	...
	tab, err := newTable(t, t.db, cfg.Revalidator, cfg.OnSeen, cfg.Bootnodes, cfg.Log)
	if err != nil {
		return nil, err
	}
	t.tab = tab
...
func (t *UDPv5) SeeNode(n *enode.Node, at time.Time, liveness uint) {
	t.tab.addSeenNode(&node{
		Node:           *n,
		addedAt:        at,
...
func (t *UDPv5) AddRecentNode(n *enode.Node, at time.Time, liveness uint) {
	t.tab.addVerifiedNode(&node{
		Node:           *n,
		addedAt:        at,
		livenessChecks: liveness,
	})
...
// PingSeq sends a ping message to the given node.
func (t *UDPv5) PingSeq(n *enode.Node) (uint64, error) {
	return t.ping(n)
}
